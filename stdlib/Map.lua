...
function table.join(...)
    ...
    for i, v in pairs({ ... }) do
        -- local v = ...[i]
        -- if (v) then
        for key, val in pairs(v) do result[key] = val end
        -- end
    end
    return result
end
...
function table.shallowJoin(t1, t2)
    ...
    local result = {}
    for key, val in pairs(t1) do result[key] = val end
    for key, val in pairs(t2) do result[key] = val end
    return result
    -- local cur = select(i, ...)
...
Map.any = function(self) for k, v in pairs(self) do return v end end
Map.anyVal = Map.any
Map.anyKey = function(self) for k, v in pairs(self) do return k end end
Map.size = function(self)
    local count = 0
    for k, v in pairs(self) do count = count + 1 end
    return count
end
Map.contains = function(self, val)
    for k, v in pairs(self) do if v == val then return true end end
    return false
end
...
function Map.deepcopy(o) return Map._deepcopy(o, {}) end

function Map.keys(o)
    local keys = {}
    for key, val in pairs(o) do table.insert(keys, key) end
    return keys
end

function Map.values(o)
    local vals = {}
    for key, val in pairs(o) do table.insert(vals, val) end
    return vals
end
