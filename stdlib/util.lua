...
function util.color(hex) -- supports 'rrggbb', 'rgb', 'rrggbbaa', 'rgba', 'ww', 'w'
    local function h(i, j)
        return j and tonumber("0x" .. hex:sub(i, j)) / 255 or
                   tonumber("0x" .. hex:sub(i, i)) / 15
    end

    hex = hex:gsub("#", "")
    return
        #hex == 6 and {r = h(1, 2), g = h(3, 4), b = h(5, 6)} or #hex == 3 and
            {r = h(1), g = h(2), b = h(3)} or #hex == 8 and
            {r = h(1, 2), g = h(3, 4), b = h(5, 6), a = h(7, 8)} or #hex == 4 and
            {r = h(1), g = h(2), b = h(3), a = h(4)} or #hex == 2 and
            {r = h(1, 2), g = h(1, 2), b = h(1, 2)} or #hex == 1 and
            {r = h(1), g = h(1), b = h(1)} or {r = 1, g = 1, b = 1}
end
...
function util.premul_color(color)
    ...
    return {r = r and (r * a), g = g and (g * a), b = b and (b * a), a = a}
end
...
function util.multiply_color(c1, n)
    return {
        (c1.r or c1[1] or 0) * (n or 0), (c1.g or c1[2] or 0) * (n or 0),
        (c1.b or c1[3] or 0) * (n or 0), (c1.a or c1[4] or 1) * (n or 1)
    }
end
...
function util.oppositedirection(direction)
    if not tonumber(direction) then
...
function util.multiplystripes(count, stripes)
    ...
    for _, stripe in ipairs(stripes) do
        for _ = 1, count do ret[#ret + 1] = stripe end
    end
    return ret
end
...
function util.foreach_sprite_definition(table_, fun_)
    ...
    fun_(table_)
    if table_.hr_version then fun_(table_.hr_version) end
    -- end
    return table_
end
...
function util.mul_shift(shift, scale)
    if not (shift and scale) then return shift end
...
function util.increment(t, k, v) t[k] = t[k] + (v or 1) end

function util.conditional_return(value, data) return value and data end
...
util.split_whitespace = function(string)
    ...
    local result = {}
    for w in string:gmatch("%S+") do table.insert(result, w) end
    return result
end
...
util.clamp = function(x, lower, upper) return
    math.max(lower, math.min(upper, x)) end
...
function util.combine_icons(icons1, icons2, inputs)
    ...
    for _, icon_to_add in pairs(icons2) do
        ...
        if icon_to_add.shift then
            icon.shift = {
                icon_to_add.shift[1] * scale + shift[1],
                icon_to_add.shift[2] * scale + shift[2]
            }
        else
            icon.shift = shift
        end
...
function util.parse_energy(energy)
    ...
    multiplier = multiplier *
                     (energy_chars[magnitude] or
                         error(magnitude .. " is not valid magnitude"))
...
function util.product_amount(product)
    return product.probability *
               (product.amount or
                   ((product.amount_min + product.amount_max) / 2))
end
...
function util.remove_tile_references(data, array_of_tiles_to_remove)
    ...
    if (type(array_of_tiles_to_remove) ~= "table") then
        error(
            "The second parameter of util.remove_tile_reference() is expected to be array of strings.")
    end

    local tiles_to_remove = {}
    for i, n in pairs(array_of_tiles_to_remove) do tiles_to_remove[n] = true end
